...
func NewStubServer() *StubServer {
	return &StubServer{
		...
		ignoredCommands: []string{"CLIENT", "HELLO", "CLUSTER"}, // these are connection setup commands
	}
}
...
func (rs *StubServer) Start(secure bool, clientCert []byte) error {
	...
	// valkey-go sends HELLO on connection to negotiate the protocol version.
	// We return a simple OK response to satisfy the handshake.
	rs.RegisterCommandHandler("HELLO", func(c *Connection, _ []string) {
		c.WriteOK()
	})

	// valkey-go sends CLUSTER commands to discover cluster topology.
	// Return an error to indicate this is not a cluster, which makes
	// valkey-go fall back to single-node mode.
	rs.RegisterCommandHandler("CLUSTER", func(c *Connection, _ []string) {
		c.WriteError(errors.New("ERR This instance has cluster support disabled"))
	})
...
func (rs *StubServer) handleCommand(c *Connection, cmd string, args []string) {
	...
	if !slices.Contains(rs.ignoredCommands, cmd) {
		rs.Lock()

...
func TestClientConstructor(t *testing.T) {
	...
	testCases := []struct {
		...
		},
		{
			...
			expErr: "invalid options; reason: valkey: invalid URL scheme: https",
		},
		{
		...
		_, err := ts.rt.RunString(fmt.Sprintf(`
			...
			redis.lpop("existing_list")
				...
				.then(
					...
					err => { if (err.error() != 'valkey nil message') { throw 'unexpected error for lpop: ' + err.error() } }
				)
		`, rs.Addr()))
		...
		_, err := ts.rt.RunString(fmt.Sprintf(`
			...
			redis.rpop("existing_list")
				...
				.then(
					...
					// An error is returned if the list does not exist
					err => { if (err.error() != 'valkey nil message') { throw 'unexpected error for rpop: ' + err.error() } }
				)
		`, rs.Addr()))
		...
		_, err := ts.rt.RunString(fmt.Sprintf(`
			...
			redis.lindex("existing_list", 0)
				...
				.then(
					...
					err => { if (err.error() != 'valkey nil message') { throw 'unexpected error for lindex: ' + err.error() } }
				)
		...
		_, err := ts.rt.RunString(fmt.Sprintf(`
			...
			redis.lset("existing_list", 0, "new_first")
				...
				.then(
					...
					err => { if (err.error() != 'no such key') { throw 'unexpected error for lset: ' + err.error() } }
				)
			`, rs.Addr()))
			...
			redis.lrem("existing_list", 1, "first")
				...
				.then(() => {
					redis.lrem("non_existing_list", 2, "third")
						.then(
							res => { if (res !== null) { throw 'unexpectedly resolved promise: ' + res } },
							err => { if (err.error() != 'no such key') { throw 'unexpected error for lrem: ' + err.error() } },
						)
				})
			`, rs.Addr()))
			...
			redis.llen("existing_list")
				...
				.then(() => {
					redis.llen("non_existing_list")
						.then(
							res => { if (res !== null) { throw 'unexpectedly resolved promise: ' + res } },
							err => { if (err.error() != 'no such key') { throw 'unexpected error for llen: ' + err.error() } }
						)
				})
			`, rs.Addr()))
	...
	gotScriptErr := ts.runtime.EventLoop.Start(func() error {
		_, err := ts.rt.RunString(fmt.Sprintf(`
			...
			redis.hset("existing_hash", "key", "value")
				...
				.then(
					res => { if (res !== null) { throw 'unexpectedly resolved promise: ' + res } },
					err => { if (err.error() != 'no such key') { throw 'unexpected error for hset: ' + err.error() } },
				)
			`, rs.Addr()))
		...
		_, err := ts.rt.RunString(fmt.Sprintf(`
			...
			redis.hget("existing_hash", "foo")
				...
				.then(
					...
					err => { if (err.error() != 'valkey nil message') { throw 'unexpected error for hget: ' + err.error() } },
				)
			`, rs.Addr()))
		...
		_, err := ts.rt.RunString(fmt.Sprintf(`
			...
			redis.hgetall("existing_hash")
				...
				.then(
					...
					err => { if (err.error() != 'valkey nil message') { throw 'unexpected error for hgetall: ' + err.error() } },
				)
			`, rs.Addr()))
		...
		_, err := ts.rt.RunString(fmt.Sprintf(`
			...
			redis.hkeys("existing_hash")
				...
				.then(
					...
					err => { if (err.error() != 'valkey nil message') { throw 'unexpected error for hkeys: ' + err.error() } },
				)
			`, rs.Addr()))
		...
		_, err := ts.rt.RunString(fmt.Sprintf(`
			...
			redis.hvals("existing_hash")
				...
				.then(
					...
					err => { if (err.error() != 'valkey nil message') { throw 'unexpected error for hvals: ' + err.error() } },
				)
			`, rs.Addr()))
		...
		_, err := ts.rt.RunString(fmt.Sprintf(`
			...
			redis.hlen("existing_hash")
				...
				.then(
					...
					err => { if (err.error() != 'valkey nil message') { throw 'unexpected error for hlen: ' + err.error() } },
				)
			`, rs.Addr()))
		...
		_, err := ts.rt.RunString(fmt.Sprintf(`
			...
			redis.srandmember("existing_set")
				...
				.then(
					...
					err => { if (err.error() !== 'no elements in set') { throw 'unexpected error for srandmember operation: ' + err.error() } }
				)
			`, rs.Addr()))
...
func TestClientZrange(t *testing.T) {
	t.Parallel()
	...
	rs.RegisterCommandHandler("ZRANGE", func(c *Connection, args []string) {
		if len(args) != 3 {
			c.WriteError(errors.New("ERR unexpected number of arguments for 'ZRANGE' command"))
			return
		}
	...
	gotScriptErr := ts.runtime.EventLoop.Start(func() error {
		_, err := ts.rt.RunString(fmt.Sprintf(`
			...
			redis.zrange("myset", "0", "-1")
				.then(res => {
					if (res.length !== 3) { throw 'unexpected length: ' + res.length }
					if (res[0] !== "member1") { throw 'unexpected value at index 0: ' + res[0] }
...
func TestClientZscore(t *testing.T) {
	t.Parallel()

	ts := newTestSetup(t)
	rs := RunT(t)
	rs.RegisterCommandHandler("ZSCORE", func(c *Connection, args []string) {
		if len(args) != 2 {
			c.WriteError(errors.New("ERR unexpected number of arguments for 'ZSCORE' command"))
			return
		}
...
func TestClientZrem(t *testing.T) {
	t.Parallel()
...
func TestClientZcard(t *testing.T) {
	t.Parallel()

	ts := newTestSetup(t)
	rs := RunT(t)
	rs.RegisterCommandHandler("ZCARD", func(c *Connection, args []string) {
		if len(args) != 1 {
			c.WriteError(errors.New("ERR unexpected number of arguments for 'ZCARD' command"))
			return
		}
...
func TestClientZrank(t *testing.T) {
	t.Parallel()

	ts := newTestSetup(t)
	rs := RunT(t)
	rs.RegisterCommandHandler("ZRANK", func(c *Connection, args []string) {
		if len(args) != 2 {
			c.WriteError(errors.New("ERR unexpected number of arguments for 'ZRANK' command"))
			return
		}
...
func TestClientZremrangebyscore(t *testing.T) {
	t.Parallel()

	ts := newTestSetup(t)
	rs := RunT(t)
	rs.RegisterCommandHandler("ZREMRANGEBYSCORE", func(c *Connection, args []string) {
		if len(args) != 3 {
			c.WriteError(errors.New("ERR unexpected number of arguments for 'ZREMRANGEBYSCORE' command"))
			return
		}

// Package valkey implements a valkey client for k6.
package valkey
...
//
// Under the hood, the valkey-go client will be used. It supports
// single-node, cluster, and sentinel modes. Depending on the options,
// the appropriate client type is used.
//
// The type of the underlying client depends on the following conditions:
// If the first argument is a string, it's parsed as a URL, and a
// single-node Client is used.
// Otherwise, an object is expected, and depending on its properties:
// 1. If the masterName property is defined, a sentinel-backed client is used.
// 2. If the cluster property is defined, a cluster client is used.
...
// The connection is automatically established when using any of the Valkey

...
# openssl creates keys as 0600, owned by the generating user. The official
# valkey image drops privileges to its `valkey` user, which then cannot read
# the mounted key. These are throwaway test certificates, so widen them.
chmod 644 tests/tls/*.key

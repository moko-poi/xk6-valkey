...
const valkeyClient = new valkey.Client(
...
export function measureRedisPerformance() {
  ...
  valkeyClient
    ...
    .then(() => valkeyClient.get(`foo-${exec.vu.idInTest}`))
    .then((value) => valkeyClient.incrBy(`foo-${exec.vu.idInTest}`, value))
    .then((_) => valkeyClient.del(`foo-${exec.vu.idInTest}`))
    .then((_) => valkeyClient.exists(`foo-${exec.vu.idInTest}`))
    .then((exists) => {
      if (exists !== 0) {
...
export function setup() {
  valkeyClient.sadd("crocodile_ids", ...crocodileIDs);
}
...
export function measureUsingRedisData() {
  ...
  valkeyClient
    ...
    .then((url) => valkeyClient.hincrby("k6_crocodile_fetched", url, 1));
}

export function teardown() {
  valkeyClient.del("crocodile_ids");
}
...
export function handleSummary(data) {
  valkeyClient
    ...
    .then((data) =>
      valkeyClient.set(`k6_report_${Date.now()}`, JSON.stringify(data))
    )
    .then(() => valkeyClient.del("k6_crocodile_fetched"));
